...
public:
	CMaterialPass(std::string strName, const SMaterialPassRenderStates* pStates,
...
private:
	...
	const SMaterialPassRenderStates* m_pStates;
	const SMaterialPassProperties* m_pProperties;
...
struct SMaterialPassProperties
{
	...
	SMaterialPassProperties(glm::vec3 clrAmbient, glm::vec3 clrDiffuse, glm::vec3 clrSpecular, float fStrength, float fShine)
	{
...
struct SMaterialPassRenderStates
{
	...
	SMaterialPassRenderStates(BLEND_TYPE blendType, bool depthTest, bool cullFace, FACE face)
	{
		BlendType = blendType;
		DepthTest = depthTest;
		CullFace = cullFace;
