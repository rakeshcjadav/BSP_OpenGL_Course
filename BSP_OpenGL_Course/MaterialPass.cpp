...
CMaterialPass::CMaterialPass(std::string strName, const SMaterialPassRenderStates* pStates,
	const SMaterialPassProperties* pProperties,
...
void CMaterialPass::SetMaterialStates() const
{
	...
	if (m_pStates->BlendType == SMaterialPassRenderStates::BLEND_TYPE::OPAQUE)
	{
		glEnable(GL_BLEND);
		glBlendFunc(GL_ONE, GL_ZERO);
	}
	else if (m_pStates->BlendType == SMaterialPassRenderStates::BLEND_TYPE::TRANSPARENT)
	{
		glEnable(GL_BLEND);
...
void CMaterialPass::SetMaterialProperties() const
{
	if (m_pProperties)
	{
...
void CMaterialPass::SetUniform(std::string name, int value) const
{
	m_pProgram->SetUniform(name, value);
}

void CMaterialPass::SetUniform(std::string name, float value) const
{
	m_pProgram->SetUniform(name, value);
}

void CMaterialPass::SetUniform(std::string name, glm::vec3& value) const
{
	m_pProgram->SetUniform(name, value);
}

void CMaterialPass::SetUniform(std::string name, glm::vec4& value) const
{
	m_pProgram->SetUniform(name, value);
}

void CMaterialPass::SetUniform(std::string name, glm::mat4& value) const
{
	m_pProgram->SetUniform(name, value);
}
